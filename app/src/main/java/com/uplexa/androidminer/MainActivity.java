        ...
        implements BottomNavigationView.OnNavigationItemSelectedListener
{
    ...
    private boolean bDisableAmayc = true; // false if active
    ...
    private void updatePayoutWidget(ProviderData d) {
        if(d.isNew) {
            ...
            sBalance = sBalance.replace("UPX", "").trim();
    ...
    public void enablePayoutWidget(boolean enable, String text) {
        ...
        if (enable) {
            ...
            TextView tvUPXUnit = findViewById(R.id.upxunit);
            tvUPXUnit.setVisibility(View.VISIBLE);
            ...
            if(tvPayoutWidgetTitle.getVisibility() != View.INVISIBLE) {
                ...
                TextView tvUPXUnit = findViewById(R.id.upxunit);
                tvUPXUnit.setVisibility(View.INVISIBLE);
    ...
    private void resetOptions() {
        bDisableAmayc = true; // false if active
        listCPUTemp.clear();
        listBatteryTemp.clear();
    ...
    private void resetHashrateTicks() {
        ...
        meterTicks.setMaxSpeed(1000);
        meterTicks.setTickNumber(0);
        meterTicks.setTextColor(getResources().getColor(android.R.color.transparent));

        meterHashrate.setMaxSpeed(1000);
        meterHashrate_avg.setMaxSpeed(1000);
        meterHashrate_max.setMaxSpeed(1000);
    }
    ...
    private void updateTemperatures() {
        ...
        if(listCPUTemp.size() >= MAX_NUM_ARRAY || listBatteryTemp.size() >= MAX_NUM_ARRAY)
        {
            ...
            if(!listCPUTemp.isEmpty() && !listBatteryTemp.isEmpty()) {
                //check2?arrayc=[35,42,45,50]&arrayb=[32,43,45,38]
    ...
    private void saveBitmap(Bitmap bitmap) {
        ...
        imagePath = new File(Environment.getExternalStorageDirectory() + "/uplexa_scrnshot.png"); ////File imagePath
    ...
    private void shareIt() {
        ...
        String shareBody = "Take a look at my uPlexa Mobile Miner stats!";
        sharingIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "My uPlexa Mobile Miner Stats");
